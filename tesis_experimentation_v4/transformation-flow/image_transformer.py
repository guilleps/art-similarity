...
def save_transform(img_path, out_dir):
    ...
    cv2.imwrite(
        os.path.join(out_dir, f"{base_name}_original.jpg"), image
    )  # guarda img_original
    ...
    cv2.imwrite(
        os.path.join(out_dir, f"{base_name}_texture.jpg"), texture
    )  # guarda img_Textura

    heatmap = apply_color_distribution_map(image)
    plt.imsave(
        os.path.join(out_dir, f"{base_name}_heatmap.jpg"), heatmap
    )  # guarda img_Mapa de calor

    hsv = apply_hsv_channels(image)
    for channel, img in hsv.items():
        cv2.imwrite(
            os.path.join(out_dir, f"{base_name}_hsv_{channel}.jpg"), img
        )  # guarda img_HSV
...
if __name__ == "__main__":
    input_dir = Path("/home/guille/workspace/data")
    output_dir = Path("/home/guille/workspace/data_transformed")
