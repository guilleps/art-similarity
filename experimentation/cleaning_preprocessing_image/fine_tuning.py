...
def train_model():
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(1e-3),
                  ...
                  metrics=['accuracy'])
    model.fit(train_ds, validation_data=test_ds, epochs=EPOCHS_PHASE1, callbacks=get_callbacks(1))
    ...
    model.fit(train_ds, validation_data=test_ds, epochs=EPOCHS_PHASE2, callbacks=[lr_scheduler] + get_callbacks(2))

    # FASE 3: Full Fine-Tuning
    model = build_model(trainable_layers=len(EfficientNetB0(weights='imagenet', include_top=False).layers))
    model.compile(optimizer=tf.keras.optimizers.Adam(1e-5),
                  loss='sparse_categorical_crossentropy',
                  metrics=['accuracy'])
    model.fit(train_ds, validation_data=test_ds, epochs=EPOCHS_PHASE3, callbacks=get_callbacks(3))
